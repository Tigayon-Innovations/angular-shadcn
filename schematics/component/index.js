...
function component(options) {
  return (tree, context) => {
    ...
    const componentPath = (0, core_1.normalize)(
      (0, core_1.join)((0, core_1.normalize)(basePath), (0, core_1.normalize)(componentName)),
    );
    context.logger.info('');
    context.logger.info(`📦 Installing ${componentName}...`);
    context.logger.info('');
    // Check if component directory already exists
    const firstFilePath = (0, core_1.join)(
      componentPath,
      (0, core_1.normalize)(componentInfo.files[0]),
    );
    ...
    for (const file of componentInfo.files) {
      let sourcePath = (0, core_1.join)(
        (0, core_1.normalize)(sourceBasePath),
        (0, core_1.normalize)(file),
      );
      ...
      if (!content) {
        sourcePath = (0, core_1.join)(
          (0, core_1.normalize)(fallbackSourcePath),
          (0, core_1.normalize)(file),
        );
    ...
    context.logger.info(
      `   import { ${toPascalCase(componentName)} } from '@/ui/${componentName}';`,
    );
    context.logger.info('');
    return tree;
  };
