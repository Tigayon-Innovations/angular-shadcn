...
console.log(
  `  Components with Examples: ${withExamples.length} / ${componentsData.length} (${Math.round((withExamples.length / componentsData.length) * 100)}%)`,
);
console.log(
  `  Components with Variants: ${withVariants.length} / ${componentsData.length} (${Math.round((withVariants.length / componentsData.length) * 100)}%)`,
);
console.log(
  `  Components with Inputs: ${withInputs.length} / ${componentsData.length} (${Math.round((withInputs.length / componentsData.length) * 100)}%)`,
);
console.log(
  `  Components with Outputs: ${withOutputs.length} / ${componentsData.length} (${Math.round((withOutputs.length / componentsData.length) * 100)}%)`,
);
console.log(
  `  Components with Related: ${withRelated.length} / ${componentsData.length} (${Math.round((withRelated.length / componentsData.length) * 100)}%)`,
);
console.log('');
...
console.log(
  `\n⚠️  ${missingNgAdd.length} Components Missing ng add command:`,
  missingNgAdd.map((c) => c.name),
);

// All components have proper installation methods
const allHaveInstall = componentsData.every(
  (c) =>
    c.installation &&
    c.installation.npm &&
    c.installation.pnpm &&
